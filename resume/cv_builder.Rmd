...
cat(skill_list)
cat("\n\n")
cat("**Selected Open Source Software:** [natverse](https://natverse.org/) • [neuromorphr](https://natverse.github.io/neuromorphr/) • [neuprintr](https://natverse.github.io/neuprintr/) • [bancr](https://github.com/flyconnectome/bancr?tab=readme-ov-file) • [insectbrainr](https://natverse.github.io/insectbrainr/)")
```

## Selected Publications __________________________________

```{r publications}
cat("\\scriptsize\n\n")  # LaTeX command for small but readable text

# Ultra-compact publication format - single line per publication
for(i in 1:min(12, nrow(publications))) {
  pub <- publications[i, ]
  clean_title <- gsub("<a href='[^']*'>([^<]*)</a>", "\\1", pub$title)
  
  # Truncate long author lists
  author_short <- pub$author
  if(nchar(author_short) > 60) {
    author_short <- paste0(substr(author_short, 1, 57), "...")
  }
  
  citation_text <- if(!is.na(pub$cites) && pub$cites > 0) glue(" ({pub$cites})") else ""
  
  cat(glue("**{clean_title}** ({pub$year}). {author_short} *{pub$journal}*{citation_text}"))
  cat("  \n")
}

if(nrow(publications) > 12) {
  cat(glue("*+{nrow(publications) - 12} more on [Scholar](https://scholar.google.com/citations?user=BOVTiXIAAAAJ&hl=en)*"))
}
