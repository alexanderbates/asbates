...
publications <- subset(publications, ! journal %in% c("CHEMICAL SENSES","University of Cambridge","Journal of Clinical Oncology",""))
# Google Scholar mislabels the PhD thesis as a "Genomics" paper; it is the thesis
# (covered under Education), so exclude it from the publication list.
publications <- subset(publications, !grepl("organisation of a third-order olfactory brain region", title, ignore.case = TRUE))

# Load full author lists from manually curated file
publications$doi <- NA_character_
if(file.exists("publications_full_citations.csv")) {
  full_pubs <- read.csv("publications_full_citations.csv", stringsAsFactors = FALSE)
  # Merge full author lists (and DOIs) based on title match
  for(i in 1:nrow(publications)) {
    title_match <- which(full_pubs$title == publications$title[i])
    if(length(title_match) > 0) {
      publications$author[i] <- full_pubs$author[title_match[1]]
      if("doi" %in% names(full_pubs)) publications$doi[i] <- full_pubs$doi[title_match[1]]
    }
  }
}
...
# Journal corrections (BANC now published in Nature; DOI is added from the
# curated citations file via doi_text, so keep journal/year clean here)
banc_corr <- grepl("Distributed control circuits across a brain-and-cord connectome", publications$title, ignore.case = TRUE)
publications$journal[banc_corr] <- "Nature"
publications$year[banc_corr] <- 2026
publications$number[banc_corr] <- ""
...
# [Alexander Shakeel Bates, PhD, FRES](https://as-bates.netlify.app/portfolio/) ![](asb_logo_round.png){width=80px style="float: right; margin-left: 20px;"}
...
contact_line <- "**Contact:** alexander_bates@hms.harvard.edu | [ORCID: 0000-0002-1195-0445](https://orcid.org/0000-0002-1195-0445) | [GitHub: alexanderbates](https://github.com/alexanderbates) | [as-bates.netlify.app](https://as-bates.netlify.app/portfolio/)"
...
tryCatch({
  ...
  if (nrow(dataverse_sources) > 0) {
    ...
    for(i in 1:nrow(dataverse_sources)) {
      ...
      cat(glue("- [{ds$citation}]({ds$url}) (DOI: {ds$doi})"))
      cat("  \n")
    }
    cat("\n")
  }
  ...
  if (nrow(zenodo_sources) > 0) {
    ...
    for(i in 1:nrow(zenodo_sources)) {
      zs <- zenodo_sources[i, ]
      cat(glue("- [{zs$citation}]({zs$url}) (DOI: {zs$doi})"))
...
for(i in 1:nrow(publications)) {
  ...
  # Bold "A. S. Bates" in author list FIRST
  author_text <- gsub("A\\. S\\. Bates", "**A. S. Bates**", author_text)
  author_text <- gsub("Alexander Shakeel Bates", "**Alexander Shakeel Bates**", author_text)

  # Special handling for papers where A. S. Bates is one of the first authors
  # Apply symbols AFTER bolding, matching abbreviated format with periods and spaces
  # ONLY add obelus to papers where A. S. Bates is actually a first author
  if(grepl("Distributed control circuits across a brain-and-cord connectome", clean_title, ignore.case = TRUE)) {
    # BANC paper: A. S. Bates†‡, J. S. Phelps†‡, M. Kim†, H. H. Yang†
    author_text <- gsub("\\*\\*A\\. S\\. Bates\\*\\*", "**A. S. Bates**\u2020\u2021", author_text)
    author_text <- gsub("J\\. S\\. Phelps", "J. S. Phelps\u2020\u2021", author_text)
    author_text <- gsub("M\\. Kim", "M. Kim\u2020", author_text)
    author_text <- gsub("H\\. H\\. Yang", "H. H. Yang\u2020", author_text)
    author_text <- gsub("M\\. Murthy", "M. Murthy‡", author_text)
    author_text <- gsub("J\\. Drugowitsch", "J. Drugowitsch‡", author_text)
    author_text <- gsub("R\\. I\\. Wilson", "R. I. Wilson‡", author_text)
    author_text <- gsub("W\\.-C\\. A\\. Lee", "W.-C. A. Lee‡", author_text)
  } else if(grepl("Sexual dimorphism in the complete connectome", clean_title, ignore.case = TRUE)) {
    # maleCNS (Berg et al.): nine co-first authors; corresponding Berg, Rubin, Jefferis
    author_text <- gsub("S\\. Berg", "S. Berg†‡", author_text)
    author_text <- gsub("I\\. R\\. Beckett", "I. R. Beckett†", author_text)
    author_text <- gsub("M\\. Costa", "M. Costa†", author_text)
    author_text <- gsub("P\\. Schlegel", "P. Schlegel†", author_text)
    author_text <- gsub("M\\. Januszewski", "M. Januszewski†", author_text)
    author_text <- gsub("E\\. C\\. Marin", "E. C. Marin†", author_text)
    author_text <- gsub("A\\. Nern", "A. Nern†", author_text)
    author_text <- gsub("S\\. Preibisch", "S. Preibisch†", author_text)
    author_text <- gsub("W\\. Qiu", "W. Qiu†", author_text)
    author_text <- gsub("G\\. M\\. Rubin", "G. M. Rubin‡", author_text)
    author_text <- gsub("G\\. S\\. X\\. E\\. Jefferis", "G. S. X. E. Jefferis‡", author_text)
  } else if(grepl("Neurotransmitter classification from electron microscopy", clean_title, ignore.case = TRUE)) {
    # Neurotransmitter paper: N. Eckstein†, A. S. Bates†
    ...
    # Olfactory connectome paper: P. Schlegel†, A. S. Bates†
    author_text <- gsub("P\\. Schlegel", "P. Schlegel\u2020", author_text)
    author_text <- gsub("\\*\\*A\\. S\\. Bates\\*\\*", "**A. S. Bates**\u2020", author_text)
  } else if(grepl("Complete connectomic reconstruction of olfactory projection neurons", clean_title, ignore.case = TRUE)) {
    # Olfactory projection neurons paper: A. S. Bates†, P. Schlegel†
    author_text <- gsub("\\*\\*A\\. S\\. Bates\\*\\*", "**A. S. Bates**\u2020", author_text)
    author_text <- gsub("P\\. Schlegel", "P. Schlegel\u2020", author_text)
  } else if(grepl("The natverse, a versatile toolbox", clean_title, ignore.case = TRUE)) {
    # Natverse paper: A. S. Bates†, J. D. Manton†
    author_text <- gsub("\\*\\*A\\. S\\. Bates\\*\\*", "**A. S. Bates**\u2020", author_text)
    author_text <- gsub("J\\. D\\. Manton", "J. D. Manton\u2020", author_text)
  }
  # NOTE: No "else" clause - for papers where A. S. Bates is NOT a first author, don't add any obelus

  # Get volume number if available (using number field from Google Scholar)
  volume_text <- if(!is.na(pub$number) && pub$number != "") glue(" {pub$number}") else ""

  # Hyperlinked DOI, from the curated citations file
  doi_text <- if(!is.null(pub$doi) && !is.na(pub$doi) && pub$doi != "") glue(" doi: [{pub$doi}](https://doi.org/{pub$doi})") else ""

  # Science bibliography style: Authors, Title. Journal Volume (Year). doi.
  cat(glue("{author_text}, **{clean_title}.** *{pub$journal}*{volume_text} ({pub$year}){doi_text}{citation_text}"))
  cat("  \n\n")
}
...
if(nrow(reviews) > 0) {
  ...
  for(i in 1:nrow(reviews)) {
    ...
    # Bold "A. S. Bates" in author list
    author_text <- gsub("A\\. S\\. Bates", "**A. S. Bates**", rev$author)
    author_text <- gsub("Alexander Shakeel Bates", "**Alexander Shakeel Bates**", author_text)

    # Get volume number if available
    volume_text <- if(!is.na(rev$number) && rev$number != "") glue(" {rev$number}") else ""

    # Science bibliography style: Authors, Title. Journal Volume (Year).
    cat(glue("{author_text}, **{clean_title}.** *{rev$journal}*{volume_text} ({rev$year}){citation_text}"))
    cat("  \n")
  }
...
**A. S. Bates**, **The lateral horn, a brain region in the fly, primes innate olfactory behaviours by combining patterns of second-order olfactory projection neuron activity.** PhD thesis, University of Cambridge (2021). [Cambridge repository](https://www.repository.cam.ac.uk/items/67fecbc9-4b86-477f-bf59-37fbaf8544fb)
