...
Behaving organisms are complicated, even one like the placozoan *Trichoplax adhaerens*, a two-dimensional mat of cells about a millimetre wide [@Smith2014-gm]. Despite lacking a nervous system it has a breadth of discrete behaviours, which we do not fully understand but which seem to involve neuropeptides and each cell's location within the mat [@Varoqueaux2018-iy]. To move a larger mass of cells into behaviours, a nervous system is needed. Complex behaviour requires many neurons in particular layouts; together with the integrative properties of those neurons, these architectures transform sensory streams, extract and compare salient features against noise, and arrange motor output, steering incoming information towards a set point [@Powers1989-lu]. Understanding how nervous systems do this in detail is very difficult.

An artificial neural network can be built, trained and run in a few lines of code. Even though a computer scientist can make any manipulation an experimental neuroscientist can, and more, it is hard to work out what features their network cares about or why it works. What matters is a grasp of cost functions, architectures, learning rules and evaluation metrics, even if one only learns which work best by trial and error [@Lillicrap2019-eo]. Investigating individual artificial neurons seems a little mad; it may be mad in neurobiology too. A cortical cell has ~10^4 connections you could probe, though most are functionally weak [@Holmgren2003-ca], and one neuron can cause a lot of chaos: a couple of spikes from a single pyramidal neuron drive the sequential activation of dozens of neurons over ~200 ms and ~1 mm² of tissue [@Hemberger2019-ku], to say nothing of the funky things neurons do through their molecular and cellular biology.
...
A good understanding of an animal's neuroanatomy needs to be complemented by a good understanding of its behavioural repertoire before we can crack the whole thing.
